...
describe("persistent AI Runtime client", () => {
  ...
  it("replays a persisted failed AI operation without invoking the provider again", async () => {
    let runtimeCalls = 0;
    ...
    const client = createPersistentAIRuntimeClient(
      {
        async execute() {
          runtimeCalls += 1;
          throw new Error("provider must not be called for failed terminal replay");
        },
      },
      {
        async ensureOperationType() {
        ...
        async createOperation() {
          return {
            id: brandId<"EntityId">("operation-failed"),
            organizationId: brandId<"EntityId">("tenant-1"),
            workspaceId: brandId<"EntityId">("workspace-1"),
            operationType: "seller.product.extract",
            operationVersion: 1,
        ...
        async getResult() {
          return {
            ...
            safetyOutcome: "allowed",
            provenance: ["none"],
            warnings: ["persisted failure"],
            errorClassification: "ProviderTimeout",
          };
        },
      } as never,
      {
