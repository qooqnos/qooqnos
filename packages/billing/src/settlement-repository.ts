...
export class SettlementRepository extends Repository {
  ...
  async approveSettlement(context: RequestContext, settlementId: EntityId, approvedBy: EntityId, now: string): Promise<SettlementRecord> {
    ...
    await this.transition(context,settlementId,"approved",now);
  ...
  private async transition(context:RequestContext,id:EntityId,status:"approved",now:string){await this.database.run("UPDATE billing_settlements SET status=?,approved_by=COALESCE(approved_by,?),updated_at=? WHERE id=? AND organization_id=? AND status='pending'",status,context.actorId??null,now,id,this.requireOrganization({organizationId:context.tenantId}));}
}
