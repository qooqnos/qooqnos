...
export interface PrivacyServiceOptions {
  readonly repository: PrivacyRepository; readonly authorization: AuthorizationService;
...
export class PrivacyService {
  ...
  async grantConsent(context:RequestContext,input:{
    readonly subjectType:PrivacySubjectType; readonly subjectId:EntityId; readonly purpose:string; readonly consentVersion:string;
    readonly source:string; readonly evidenceReference?:string; readonly grantedAt?:string; readonly expiresAt?:string;
  }){
  ...
  async revokeConsent(context:RequestContext,id:EntityId){
    ...
    return this.options.repository.revokeConsent(context,id,now,now);
  }

  async createRequest(context:RequestContext,input:{
    readonly subjectType:PrivacySubjectType; readonly subjectId:EntityId; readonly requestType:"access"|"export"|"delete"|"restrict"|"correct";
    readonly requestedBy:string; readonly dueAt?:string;
  }){
  ...
  async recordProcessing(context:RequestContext,input:{
    readonly requestId:EntityId; readonly moduleId:string; readonly action:string; readonly resourceReference?:string;
    readonly status:"queued"|"processing"|"completed"|"failed"|"skipped"; readonly errorReference?:string; readonly processedAt?:string;
  }){
    await this.options.authorization.assert({context,permission:"privacy.request.manage",requireAuthentication:true,requireWorkspace:false});
    return this.options.repository.recordProcessing(context,{...input,id:this.options.id(),now:this.options.now()});
  }
}

export const PRIVACY_PERMISSIONS=["privacy.consent.read","privacy.consent.manage","privacy.request.read","privacy.request.manage"] as const;
