import { AppError, type EntityId, type RequestContext } from "@qooqnos/core";

export type Permission = string;
export type Role = string;
export type MembershipStatus = "invited" | "pending" | "active" | "suspended" | "removed";

export interface RoleDefinition { readonly id: Role; readonly permissions: readonly Permission[]; }
...
export function createAuthorizationRegistry(permissions: Readonly<Record<Permission, ResourcePolicy | undefined>> = {}, roles: readonly RoleDefinition[] = [], entitlementEvaluator?: EntitlementEvaluator): AuthorizationRegistry { const registry = new AuthorizationRegistry(entitlementEvaluator); for (const [permission, policy] of Object.entries(permissions)) registry.registerPermission(permission, policy); for (const role of roles) registry.registerRole(role); return registry; }
