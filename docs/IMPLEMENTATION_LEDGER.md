...
Migration lock note: migration 0021 was refreshed before provisioning after a pre-apply SQL cleanup; migrations 0022–0054 are registered and locked in sequence from canonical SQL contents. Migration 0054 checksum was independently reconciled from canonical SQL. The verification script also checks API migration import order and migrationSources order against the canonical SQL sequence. The the current head `1e626926adaa72f8ca7823e546f84710d6b1478c` passed both CI and Phoenix verification. Full external D1 application has not yet been executed.
